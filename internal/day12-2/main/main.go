...
func (bs *BlockSet) findSetFromRemainder(buildString *string, blocks []*Block, combos []int) (int, error) {
	...
	for blockIndex := 0; blockIndex < len(blocks); blockIndex++ {
		...
		if fitX <= charLength {
			for i := 0; i <= charLength-fitX; i++ {
				...
				if count, err := bs.findSetFromRemainder(&clone, newBlocks, combos[1:]); err == nil {
					//noValidCombo = false
					resultCount += count
				}
			}
