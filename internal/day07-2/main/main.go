...
//Last attempt:  250401978
//Last attempt2: 247230943
...
func extractHandSummary(hand string) CardHand {
	...
	if jokerCount > 0 {
		var highestCountCard CardValue
		for _, cardValue := range cardSet {
			if cardMap[cardValue] > highestCount {
				highestCount = cardMap[cardValue]
				highestCountCard = cardValue
			}
		}
